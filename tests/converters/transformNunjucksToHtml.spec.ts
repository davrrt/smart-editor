import { Variable, Condition, Loop, SignatureZoneEditorMeta } from "src";
...
describe('transformNunjucksToHtml', () => {
  ...
  const signatures: SignatureZoneEditorMeta[] = [
    {
      id: 'sig-1', // doit matcher data-id
      signerKey: 'sign.user', // doit matcher data-signer-key
  ...
  it('transforme une variable nunjucks en span HTML', () => {
    const raw = '{{ Company }}';
    const output = transformNunjucksToHtml(raw, [], [], variables, []);
  ...
  it('transforme une condition nunjucks en HTML avec data-nunjucks-if', () => {
    ...
    const output = transformNunjucksToHtml(raw, conditions, [], [], []);
  ...
  it('transforme une boucle nunjucks en HTML avec data-nunjucks-for', () => {
    ...
    const output = transformNunjucksToHtml(raw, [], loops, [], []);
  ...
  it('transforme une signature zone HTML existante avec bon label et alignement', () => {
    ...
    const output = transformNunjucksToHtml(raw, [], [], [], signatures);
    expect(output).toContain('data-id="sig-1"');
    expect(output).toContain('data-signer="Signataire"');
    expect(output).toContain('data-label="Signataire"');
    expect(output).toContain('data-signer-key="sign.user"');
    expect(output).toContain('data-alignment="right"');
  ...
  it('préserve les balises <style>', () => {
    ...
    const output = transformNunjucksToHtml(raw, [], [], [], []);
